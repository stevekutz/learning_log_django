...
admin.site.register(Topic)  # registers Topic to be managed via admin site
~~~
...
1) From the virtual environment launched in terminal, create the users app
    `python manage.py startapp users`
- A similar directory structure to the `learning_logs` app is created
    <img src = "users_app.png">
2) Add the users app to `INSTALLED_APPS` in `settings.py`
        ~~~ py
        INSTALLED_APPS = [
            'django.contrib.admin',
            'django.contrib.auth',
...
1) Create a new `urls.py` file
    ...
    - the `path` & `include` function must be imported in order to implemement Django's default authentication
...
{% endblock content %}
~~~
3) `IMPORTANT` The logout page will not be shown and the admin panel logged out message will appear unless the apps you created come before `django.contrib.admin`  within `INSTALLED_APP` inside of `settings.py`
    ~~~ py
    INSTALLED_APPS = [
        # apps I created must come before django.contrib.admin in order for logout page to be shown, otherwise admin logout is shown 
        'learning_logs',
        'users', 
...
### Connnect data to specific users
...
4) Update index.html with the following
    ...
    {% block page_header %}
        ...
        <div class = "jumbotron">
            ...
            topics you've learned about. Whenever you learn something new about a topc,
